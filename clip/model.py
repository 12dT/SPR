...
class ResidualAttentionBlock_PECPI(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
        ...
        self.compound_prompt_nctx = design_details['pecpi_length']
        if i == 0:
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        elif current_trainer == 'PECPI':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_PECPI(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        else:
...
class VisionTransformer_PECPI(nn.Module):
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "PECPI":
                self.visual = VisionTransformer_PECPI(
                    input_resolution=image_resolution,
